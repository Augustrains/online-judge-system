...
{  
   ...
   class Server
   {
    ...
    static void SelectAll( const httplib::Request &req,httplib::Response &rsp)
    {
       ...
       Json::Value inform;
     if(tb_video->SelectAll(id,name,inform)==false)
    {
     rsp.status=500;
    ...
    static void SelectOne( const httplib::Request &req,httplib::Response &rsp)
    {
      ...
      int idd = std::stoi(req.matches[1]);
      std::string name=req.matches[2]; 
      Json::Value inform;
    if(tb_video->SelectOne(idd,name,inform)==false)
   {
    rsp.status=500;
    rsp.body=R"({"result":false,"reason":"查找指定id失败"})";
    rsp.set_header("Content-Type","application/json");
  ...
  bool clearFile(const std::string& filePath) {
    ...
    bool RunModule()
    {
      ...
      _svr.set_mount_point("/",WWWROOT);
       ...
       _svr.Get("/select/(\\d+)(?:/([^/]+))?", SelectAll); // ?: 表示非捕获组，? 表示可选
       _svr.Get("/code/(\\d+)(?:/([^/]+))?",SelectOne);
       ...
       std::string content =video["content"].asCString() ;
        ...
        content="";
        Json::Value inform;
        std::string command = "python3.8  test.py"; 
        // 执行命令
